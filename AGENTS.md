...
Hub Events Agent — это AI-агент на базе Gemini API, который:
...
Агент определяет регион пользователя через детерминированный mapping в `src/lib/filter.ts`. Gemini может помочь понять свободный текст, но финальная фильтрация данных использует region key:
...
После получения сырых постов из Apify, Gemini вызывается повторно для структурированного извлечения данных:
